...
int main (int argc, const char * argv[]) {
    ...
    if(argc == 2)
        ...
        filename = "gutenberg.txt"; /*specify your input text file here*/
	...
	while(!feof(fileptr)){
		char * c = getWord(fileptr);
		if(c){
		if(containsKey(hashTable, c)){
			int x = atMap(hashTable,c);
			x++;
		   insertMap(hashTable,c,(x));
		}
		else
...
//while(getWord(fileptr) != NULL)
	//	 insertMap(hashTable, getWord(fileptr),1);
